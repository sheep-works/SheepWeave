...
- [x] フォルダを開く（open）
    - [x] 現状はシステムエクスプローラーではなく、VS Codeのツリーが強調表示されるだけなので、要改善
- [x] フォルダの準備（init）
    - [x] Archiveフォルダの作成
    - [x] Workingフォルダの退避
- [x] ファイルの準備（Prepare）
...
- [x] 予測語の登録
    - [x] TBをインテリセンスで使えるようにする
- [x] スニペットの登録
    - [x] プロジェクト全体で`phrase.json`を共有し、スニペット的に使えるようにする
- [ ] AI 予測の使用
    - [ ] ゴーストテキストで `// IMPORTANT! DO NOT CHANGE LINE NUMBERS OR LINE BREAKS` を入れて、AI 補完による行破壊を予防
    - [ ] さらに補完適用時の行破壊を検知した際に、現在行以外への波及を防ぐ
- [x] フィルタ機能
    - [x] WebView にフィルタ機能を実装。所定の条件にあった行のみを抜き出して、エディタ外で編集。適用時は data とエディタ両方に反映する。
...
4. [ ] エディタの機能拡張
    ...
    - [x] 4.3 予測語の登録
    - [x] 4.4 スニペットの登録
    - [x] 4.5 フィルタ機能（Webview側にて）
    - [ ] 4.6 AI 予測の使用
5. [ ] 外部連携
    - [ ] 5.1 SheepSpindle による QA
    - [ ] 5.2 SheepLint とのシームレス連携（SheepHub）
6. [ ] データ管理
