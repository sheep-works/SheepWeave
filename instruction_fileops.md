...
- preprocessor 関数
    ...
    - convXlf2Json：Working/03_XLF_JSONにコピーされた拡張子がxlf、xliff、mxliffのファイルをClass ShWvDataにパースし、Working/03_XLF_JSON/<file_name>.jsonとして出力しておく。ShWvData Classについては、sample/ShWvData.tsを参考にして実装する。xlfファイルの構造とパース案については、sample/xlfSchema.tsとxlfreader.tsを参考にすること。
    - convJson2Shwv：ShWvDataから原文ファイル（shwvs）と訳文ファイル（shwvt）を生成し、Working/04_SHWV へ出力する。

- postprocessor 関数
    - 以下の関数を順次実行する
    - convShwv2Json：Working/04_SHWVにコピーされたshwvtファイルを使って、Class ShWvDataおよびWorking/03_XLF_JSON/<file_name>.jsonを更新する。
    - convJson2Xlf：Working/03_XLF_JSON/<file_name>.xlf/xliff/mxliffをxml2jsでパースし、ShWvDataの内容で更新したものを、再度XML形式に戻してWorking/05_COMPLETED/<file_name>-done.xlf/xliff/mxliffとして出力する。