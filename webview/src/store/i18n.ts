    ...
    state: () => ({
        ...
        tabs: {
            ...
            llmTab: {
                ...
                pinnedTabTitle: { en: "Pinned PE Refs", ja: "ピン留め (PE)", zh: "固定 PE 参考" },
                pinnedCard: { en: "Pinned Segments for Advanced PE", ja: "Advanced PE 用ピン留め一覧", zh: "Advanced PE 固定分句列表" },
                pinnedHelp: { en: "Segments pinned here are automatically provided as high-priority reference examples during Advanced PE mode.", ja: "ここでピン留めされたセグメントは、Advanced PE モード実行時にAIへ最優先の修正手本（PE参照例）として提示されます。", zh: "此处固定的分句将在 Advanced PE 模式下作为最高优先级的修改示例提供给 AI。" },
                noPinnedSegments: { en: "No pinned segments. Pin segments from the Translate tab or editor to use them as Advanced PE references.", ja: "ピン留めされたセグメントはありません。翻訳タブやエディタでピンアイコンをクリックして登録できます。", zh: "暂无固定的分句。在翻译标签页或编辑器中点击固定图标即可添加。" },
                unpinBtn: { en: "Unpin", ja: "ピン解除", zh: "取消固定" },
                jumpToLine: { en: "Jump to Line", ja: "エディタで移動", zh: "在编辑器中跳转" },
                ...
                applyToPreLabel: { en: "Also reflect to pre (draft)", ja: "下訳 (pre) にも反映", zh: "同时反映至下译 (pre)" },
                ...
                idxZeroNote: { en: "Note: Line numbers (idx) are 0-indexed and may be 1 less than the editor line numbers.", ja: "※ 行番号 (idx) は 0 始まりのため、エディタの行番号より 1 小さい場合があります。", zh: "※ 行号 (idx) 从 0 开始，可能比编辑器的行号小 1。" },
            ...
            toolsTab: {
                ...
                reviewSubtab: { en: "Review", ja: "レビュー", zh: "审校" },
                exportReviewHtmlCard: { en: "Review HTML Export", ja: "レビュー用HTML出力", zh: "审校 HTML 导出" },
                exportReviewHtmlHelp: { en: "Export a standalone HTML reader (Review_Terms.html) with registered terms highlighted and paragraph restructuring. Ideal for proofreading and reading flow review.", ja: "登録用語がハイライトされ段落が再構成された静的HTML (Review_Terms.html) を出力します。ブラウザで安全に全件レビューや印刷が可能です。", zh: "导出带有术语高亮和段落重构的独立静态 HTML (Review_Terms.html)。便于在浏览器中进行全文审校与打印。" },
                exportReviewHtmlBtn: { en: "Export Review HTML", ja: "レビュー用HTMLを出力", zh: "导出审校 HTML" },
            ...
            settingsTab: {
                ...
                termLineRange: { en: "Term Highlight Range (Lines)", ja: "用語ハイライト範囲（前後行数）", zh: "术语高亮范围（前后行数）" },
                termLineRangeTooltip: { en: "Number of lines before and after active line to highlight terms (0: current line only)", ja: "カーソル行を中心としたハイライト対象の行範囲（0: 現在行のみ、2: 前後2行）", zh: "以光标行为中心的术语高亮范围（0: 仅当前行、2: 前后2行）" },
                termColor: { en: "Term Highlight Color", ja: "用語ハイライトの文字色", zh: "术语高亮颜色" },
                termColorTooltip: { en: "Font color used for highlighting registered terms", ja: "登録用語のハイライト表示に使用する文字色", zh: "注册术语高亮显示的字体颜色" },
